...
def sample_best_team(rd, pops, networks):
    ...
    for cf_id in range(p["n_configurations"]):
        ...
        for step_id in range(p["steps"]):
            ...
            rover_actions = []
            for rv in rd.rovers:
                action = networks[f'NN{rd.rovers[rv].rover_id}'].run_network(rd.rovers[rv].observations)
...
def rover_global():
    ...
    for rover_id in range(p["n_rovers"]):
        ...
        networks[f'NN{rover_id}'] = TorchNetwork(n_inp=p["n_inp"], n_hid=p["n_hid"], n_out=p["n_out"])
